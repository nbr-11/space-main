...
function ServerSearch({ data }: ServerSearchProps) {
    ...
    return (
        <>
            <button onClick={() => setIsOpen(true)} className='group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zince-700/50 transition'>
                            ...
                            return <CommandGroup key={label} heading={label}>
                                {
                                    data?.map(({ id, icon, name }) => {
                                        return <CommandItem onSelect={() => onClick({ id, type })} key={id}>
                                            {icon}
                                            <span>{name}</span>
                                        </CommandItem>
                                    })
